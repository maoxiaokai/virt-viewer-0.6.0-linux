...
extern struct remote_viewer_data rvdata; /*@add by yl: custom data to be received*/
...
virt_viewer_auth_collect_credentials(GtkWindow *window,
                                     ...
                                     char **password)
{
	/*@yl:do not show the widget, auto set the password var to be reterned*/
	*password = g_strdup(rvdata.control_password);
	printf("virt_viewer_auth_collect_credentials:  %s\n", *password);
	return 0;
	...
	//@mao
        message = g_strdup_printf("验证连接信息");
    }
