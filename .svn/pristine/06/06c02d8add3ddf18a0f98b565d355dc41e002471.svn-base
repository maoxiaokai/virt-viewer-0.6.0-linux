...
extern struct remote_viewer_data rvdata; /*@modify by yl: custom data to be received*/
extern int connectflag;
extern int cfd;  /*used to socket comunicating*/
    ...
    if (ask) {
        GtkWidget *dialog =
            gtk_message_dialog_new (virt_viewer_window_get_window(window),
                    ...
                    _("\n确定需要关闭远程连接？ "));
//@mao
        GtkWidget *check = gtk_check_button_new_with_label(_(" 不在询问"));
...
virt_viewer_app_activate(VirtViewerApp *self, GError **error)
{
	...
	//@mao
        virt_viewer_app_show_status(self, _("正在连接图形服务器"));
                          ...
                          VirtViewerApp *self)
{
    ...
    //    virt_viewer_app_show_status(self, _("Connected to graphic server"));
		virt_viewer_app_show_status(self, _("连接到图形服务器"));
}
...
////////////////////////////////////////////////
	char writeMsg[20];
	int virtViewerPid = getpid();

    if (connect_error) {
	//viewer界面屏蔽连接失败弹框，设置连接标志位即可
//        virt_viewer_app_simple_message_dialog(self,
  //                                            _("Unable to connect to the graphic server %s"),
    //                                          priv->pretty_address);
	connectflag = 1;
	strcpy(writeMsg, "false");

    }
else
	strcpy(writeMsg, "true");
	sprintf(writeMsg+strlen(writeMsg),"%d",virtViewerPid);
	write(cfd, writeMsg, 20);
	close(cfd);
...
virt_viewer_app_update_pretty_address(VirtViewerApp *self)
{
	...
	//priv->pretty_address = g_strdup(priv->guri);
        //priv->pretty_address = g_strdup(buffer);
    priv->pretty_address = g_strdup(rvdata.ip_str); /*@modify by yl*/
    else if (priv->gport)
