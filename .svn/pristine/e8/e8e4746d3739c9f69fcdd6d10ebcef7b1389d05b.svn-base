...
//@mao
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("断开连接"));
...
//@mao
    gtk_tool_button_set_label(GTK_TOOL_BUTTON(button), _("选择USB设备"));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("选择USB设备"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM(button), 0);
...
//@mao	
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("发送组合键"));
    gtk_widget_show(GTK_WIDGET(button));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM(button), 0);
...
//    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM(button), 0);
	...
	/////////////////////////////
	fixed = gtk_fixed_new();//函数用于创建新的固定容器
    gtk_container_add(GTK_CONTAINER(priv->layout), fixed);
	
	gtk_fixed_put(GTK_FIXED(fixed), priv->toolbar, 50, 0);//函数将priv->toolbar放在fixed的由x和y指定的位置。
}
