...
//@modify by yl
extern struct remote_viewer_data rvdata;
...
connect_dialog(gchar **uri)
{
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		//*uri = g_strdup(buffer);
		*uri = g_strdup(rvdata.ip_str);  /*@modify by yl*/
        g_strstrip(*uri);
        retval = 0;
    } else {
        *uri = NULL;
...
retry_dialog:
        if (priv->open_recent_dialog) {
			...
			//guri = g_strdup(buffer);
			guri = g_strdup(rvdata.ip_str);  /*@modify by yl*/
			g_object_set(app, "guri", guri, NULL);
		...
		//@mao
         //   virt_viewer_app_simple_message_dialog(app, _("Cannot determine the connection type from URI"));
			virt_viewer_app_simple_message_dialog(app, _("不能确定的连接类型"));
            goto cleanup;
        }
#ifdef HAVE_OVIRT
